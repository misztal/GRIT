#ifndef EROSION_MAKE_CORE_OBJECT_H
#define EROSION_MAKE_CORE_OBJECT_H
...
namespace erosion
{
   ...
   */
  inline CoreObject make_core_object(
                                      grit::engine2d_type const & engine
                                      , unsigned int const & core_label
                                      , bool const & use_normals = true
                                      )
  {
    CoreObject core;

    core.m_label = core_label;

    grit::SimplexSet const liquid_set     = grit::compute_phase(engine.mesh(), core_label);
    grit::SimplexSet const interface_set  = grit::compute_interface(engine.mesh(), liquid_set );

    core.m_liquid       = glue::make_phase(engine, liquid_set);
    core.m_interface    = glue::make_phase(engine, interface_set);

    core.m_interface.m_labels.push_back(core_label);
    ...
    if(use_normals)
    {
      details::compute_edge_normals(
                                    ...
                                    , core_label
                                    , interface_set
                                    , core.m_interface_edge_normals_x
                                    , core.m_interface_edge_normals_y
                                    );
      ...
      details::compute_vertex_normals(
                                      ...
                                      , core_label
                                      , interface_set
                                      , core.m_interface_vertex_normals_x
                                      , core.m_interface_vertex_normals_y
                                      , core.m_interface_vertex_strict_convex
                                      , core.m_interface_vertex_theta
                                      );
    }
