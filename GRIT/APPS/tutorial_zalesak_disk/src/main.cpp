...
{
  std::string const filename = output_path + util::generate_filename("/tutorial_zalesak_disk", frame_number, "svg");
...
void do_simulation_step()
{
  ...
  for( unsigned int n = 0; n < N; ++n)
  {
    double const x = px[n];
    double const y = py[n];
    double const u = -(y - 0.5);
    double const v =  (x - 0.5);
...
int main()
{
  if(!settings.load("tutorial_zalesak_disk.cfg"))
  {
    return 1;
  }

  std::string  const txt_filename = settings.get_value("txt_filename",  "zalesak.txt");
