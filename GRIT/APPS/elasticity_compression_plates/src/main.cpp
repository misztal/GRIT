...
int main()
{
  if(!settings.load("elasticity_compression_plates.cfg"))
  {
    return 1;
  }

  std::string  const input_filename = settings.get_value("input_filename","breast.msh");
  ...
  grit::init_engine_with_mesh_file( util::get_data_file_path(input_filename), parameters, engine);
