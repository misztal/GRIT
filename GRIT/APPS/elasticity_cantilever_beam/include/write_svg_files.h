                     ...
                     )
{
  ...
  std::string const filename_spatial = output_path + util::generate_filename("/elasticity_cantilever_beam_spatial", frame_number, "svg");

  glue::svg_draw(filename_spatial, engine, parameters);

  std::string const filename_material = output_path + util::generate_filename("/elasticity_cantilever_beam_material", frame_number, "svg");
