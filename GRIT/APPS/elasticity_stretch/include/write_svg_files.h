                     ...
                     )
{
  ...
  std::string const filename_spatial = output_path + util::generate_filename("/elasticity_stretch_spatial_", frame_number, "svg");

  glue::svg_draw(filename_spatial, engine, parameters);

  std::string const filename_material = output_path + util::generate_filename("/elasticity_stretch_material_", frame_number, "svg");
