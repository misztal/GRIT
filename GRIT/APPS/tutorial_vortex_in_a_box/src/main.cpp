...
{
  std::string const filename = output_path + util::generate_filename("/tutorial_vortex_in_a_box", frame_number, "svg");
...
void do_simulation_step()
{
  ...
  for( unsigned int n = 0; n < N; ++n)
  {
    ...
    double u =   2.0 * cos( pi * y ) * sin( pi * y ) * sin( pi*x ) * sin( pi*x );
    double v = - 2.0 * cos( pi * x ) * sin( pi * x ) * sin( pi*y ) * sin( pi*y );
...
int main()
{
  if(!settings.load("tutorial_vortex_in_a_box.cfg"))
  {
    return 1;
  }
