                            ...
                            )
{
  ...
  unsigned int const label         = util::to_value<unsigned int>( settings.get_value("liquid_label", "1!"  )  );

  liquid::Parameters const controls = liquid::make_parameters_from_config_file(settings);

  setup_simulation_fields( engine );

  for (size_t i = 0; i <= frames; ++i)
  {
    do_simulation_step(engine, label, controls);
